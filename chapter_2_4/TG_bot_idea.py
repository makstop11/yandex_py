...
def get_range_user_comp():
    ...
    left_range = is_valid_digit(input('👉: '))  # Пользователь вводит число, и сразу идет проверка на корректность вода (цифра/буква/символ)

    time.sleep(1)
    print('Какое будет конечное число❓')
    while True:
        right_range = is_valid_digit(input('👉: '))  # Пользователь вводит число, и сразу идет проверка на корректность вода (цифра/буква/символ)
...
def user_game(popit, l_range, r_range):
    ...
    for _ in range(popit):  # кол-во итераций == кол-ву введенных пользователем попыток
        print('😈Отгадывай😈')
        clue = is_valid_digit(input('👉: '))  # Пользователь вводит число, и сразу идет проверка на корректность вода (цифра/буква/символ)
        time.sleep(1)
...
def computer_game(l_range, r_range):
    ...
    for _ in range(pop):  # Кол-во итераций == кол-ву попыток указанных компьютером
        ...
        if answer in start_list and answer != '+':  # Если пользователь подтверждает угадывание числа, завершаем игру, а компьютре радуется победе
            count_pop += 1
            print(random.choice(['Еееессс, я угадал с', 'Вот так❗️ ДА❗️ Учииись😝 Я угадал с']), count_pop, 'попытки❗️❗️❗️')
            break
